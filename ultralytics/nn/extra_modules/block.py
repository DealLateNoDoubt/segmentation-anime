...
__all__ = ["Fusion", "ScConv", "GLSA", "C2f_Star", "C2f_Attention"]
...
class SRU(nn.Module):
    def __init__(self,
        ...
        super().__init__()
        print("oup_channels", oup_channels, group_num)
        self.gn = nn.GroupNorm(num_channels=oup_channels, num_groups=group_num) if torch_gn else \
            GroupBatchnorm2d(c_num=oup_channels, group_num=group_num)
...
class ScConv(nn.Module):
    ...
    （空间和通道重建卷积）的卷积模块，目的是减少卷积神经网络中特征之间的空间和通道冗余，从而压缩 CNN 模型并提高其性能
    """
    def __init__(
            self,
            op_channel: int,
...
# region GLSA-Global-to-Local Spatial Aggregation Module start
class GLSA(nn.Module):
    """
    https://github.com/Barrett-python/DuAT  PS(用于医学图像分割)
    """
    ...
    def __init__(
        ...
        self.GlobalBlock = ContextBlock(inplanes=embed_dim, ratio=2)
    ...
    def forward(self, x):
        ...
        # Global block
        Global = self.GlobalBlock(self.global_11conv(x_1))
...
class GLSAConvBranch(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None):
        ...
        self.ca = GLSAChannelAttention(64)  # 通道注意力
        self.sa = GLSASpatialAttention()    # 空间注意力
...
class C2f_Attention(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        ...
        self.cv1_att = EMA(2 * self.c)
        self.cv2 = Conv((2 + n) * self.c, c2, 1)  # optional act=FReLU(c2)
        self.m = nn.ModuleList(Bottleneck(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0) for _ in range(n))

    def forward(self, x):
        """Forward pass through C2f layer."""
        y = list(self.cv1_att(self.cv1(x)).chunk(2, 1))
